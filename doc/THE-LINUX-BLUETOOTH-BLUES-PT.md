...
de conseguir uma versão `beta` do `Eutherpe`.
...
sink**. Às vezes (só *às vezes*, não sempre) restartar o serviço do `pulseaudio` "resolvia"
comigo.
...
melhorar ou pôr a perder todo o seu esforço de entregar algo utilizável. Eu vou mais além,
...
de ir entulhando os arquivos prefixando-os com número (sem comentários)... Logo vi que ele não
...
>`iPod Shuffle`, cuja a última atualização parou no `iOS 9`, dá um banho no `PulseAudio`,
> `PipeWire` e `WirePlumber` juntos. O subsistema `bluetooth` dele conecta em qualquer
> dispositivo `Bluetooth` de saída, não engasga o som e faz o que se propõe, toca música
> sem enrolação. Uma perfeição! Apple, hein? Que dizem ter fama de só funcionar com Apple...
> Pensa aí...
...
básico de sempre conectar no `Bluetooth` e não engasgar. Absolutamente ridículo! Hoje em dia
acho que todo sistema operacional digno de ser usado em `desktop` é obrigatório que tenha um
sistema de som excepcional. Quase todo usuário final supervaloriza som! Eu,
desenvolvedor de `software`, amo música, não programo sem, um computador pessoal que não toca
música para mim é um caiau bizarro!
...
> adiante e resolva a vida e não o contrário... Solução que cria problema, não é solução. É melhor
> não funcionar nunca e me libertar para buscar algo melhor ou funcionar sempre. Nisso é preciso
> ser 8 ou 80.
